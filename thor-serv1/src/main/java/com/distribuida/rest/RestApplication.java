...
//                       .read("secret/customer")
//                       .getData().get("thorntail.datasources.data-sources.customerDataSource.password");
//        System.out.format( "pruebaVault key in secret/customer is " + value +"\n");
//       
//       System.setProperty("thorntail.datasources.data-sources.customerDataSource.password", value);
...
//		
		...
		ServiceInstance<Object> serviceInstance = ServiceInstance.builder().id("CustomersID:" + (port + offset))
				.name("Customers").port(port + offset).address(IP_local)
				.uriSpec(new UriSpec("{scheme}://{address}:{port}")).build();

		ServiceDiscoveryBuilder.builder(Object.class).basePath("services").client(curatorFramework)
